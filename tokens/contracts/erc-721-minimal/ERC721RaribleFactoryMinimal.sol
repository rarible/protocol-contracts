 ...
 *
 */
contract ERC721RaribleFactoryMinimal is Ownable {
    ...
    function createPublicToken(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI) external {
        bytes memory data = abi.encodeWithSelector(ERC721RaribleMinimal(0).__ERC721Rarible_init.selector, _name, _symbol, baseURI, contractURI, transferProxy, lazyTransferProxy);
        BeaconProxy beaconProxy = new BeaconProxy(address(beacon), data);
        ERC721RaribleMinimal(address(beaconProxy)).transferOwnership(_msgSender());
    ...
    function createPrivateToken(string memory _name, string memory _symbol, string memory baseURI, string memory contractURI, address[] memory operators) external {
        bytes memory data = abi.encodeWithSelector(ERC721RaribleMinimal(0).__ERC721RaribleUser_init.selector, _name, _symbol, baseURI, contractURI, operators);
        BeaconProxy beaconProxy = new BeaconProxy(address(beacon), data);
        ERC721RaribleMinimal(address(beaconProxy)).transferOwnership(_msgSender());
