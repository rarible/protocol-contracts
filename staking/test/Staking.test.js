...
contract("Staking", accounts => {
 ...
 function balanceOfEventHandler(resultBalanceOfMethod) {
  ...
  function splitEventHandler(resultSplitMethod) {
  	let idLock1;
		let idLock2;
    truffleAssert.eventEmitted(resultSplitMethod, 'SplitTest', (ev) => {
     	idLock1 = ev.idFirst;
     	idLock2 = ev.idSecond;
      return true;
    });
	...
	describe("Part1. Check base metods Staking contract, createLock, withdraw", () => {
      ...
      truffleAssert.eventEmitted(resultTotalBalance, 'totalBalanceResult', (ev) => {
		...
		it("Test4.1 Try to createLock() and check withdraw(), with cliff", async () => {
			await token.mint(accounts[2], 100);
   		await token.approve(staking.address, 1000000, { from: accounts[2] });
		...
		it("Test4.2 Try to createLock() and check withdraw(), with cliff and tail", async () => {
			await token.mint(accounts[2], 100);
   		await token.approve(staking.address, 1000000, { from: accounts[2] });

			resultLock  = await forTest._stake(staking.address ,accounts[2], accounts[2], 36, 10, 3);
			/*3 week later nothing change, because cliff */
			await increaseTime(WEEK * 3);
			staking.withdraw({ from: accounts[2] });
      resultBalanceOfValue = await forTest._balanceOf(staking.address, accounts[2]); //for check balance accounts[4]
      balanceOf = balanceOfEventHandler(resultBalanceOfValue);
      assert.equal(balanceOf, 36);    //stRary balance accounts[2]
      ...
      resultBalanceOfValue = await forTest._balanceOf(staking.address, accounts[2]); //for check balance accounts[4]
      balanceOf = balanceOfEventHandler(resultBalanceOfValue);
      assert.equal(balanceOf, 26);    //stRary balance accounts[2]

 			assert.equal(await token.balanceOf(staking.address), 26);	//balance Lock ondeposite
   		assert.equal(await token.balanceOf(accounts[2]), 74);			//tail user balance
		});
	...
	describe("Part5. Check delegate()", () => {
      ...
      staking.withdraw({ from: accounts[2] });
		  ...
		  await forTest._depute(staking.address, idLock, accounts[4]);  //delegate from accounts[3]
      ...
      assert.equal(balanceOf, 0);     //stRary balance accounts[3], after _depute
      ...
      assert.equal(balanceOf, 20);    //stRary balance accounts[4], after _depute
      ...
      assert.equal(balanceOf, 3);    //stRary balance accounts[4], after _depute
      ...
      assert.equal(balanceOf, 63);    //stRary balance accounts[4], after _depute
      ...
      assert.equal(balanceOf, 43);    //stRary balance accounts[4], after _depute
      ...
      staking.withdraw({ from: accounts[2] });
		  ...
		  await forTest._depute(staking.address, idLock, accounts[4]);  //delegate from accounts[3] to accounts[4]
      resultBalanceOfValueAccount_3  = await forTest._balanceOf(staking.address, accounts[3]); //for check balance accounts[3]
      truffleAssert.eventEmitted(resultBalanceOfValueAccount_3, 'balanceOfResult', (ev) => {
      	balanceOf = ev.result;
        return true;
      ...
      assert.equal(balanceOf, 20);    //stRary balance accounts[3], after _depute()

  		await increaseTime(WEEK*5); //5 week later
		  await forTest._depute(staking.address, idLock, accounts[3]);  //delegate from accounts[4] to accounts[3] (delegate back)
      resultBalanseOfValue  = await forTest._balanceOf(staking.address, accounts[4]);
      truffleAssert.eventEmitted(resultBalanseOfValue, 'balanceOfResult', (ev) => {
      	balanceOf = ev.result;
        return true;
      ...
      await increaseTime(WEEK*30); //20 week later
		  await expectThrow(
		    forTest._depute(staking.address, idLock, accounts[4])  //delegate from accounts[3]
		  );
		});
  })
	...
	describe("Part9. Check events emit()", () => {
      ...
      await staking.stake(accounts[2], accounts[3], 20, 10, 7);
			resultDelegate  = await staking.depute(id, accounts[4]);
	...
	describe("Part10. Check split()", () => {
		it("Test.1. split() and check balance delegated stRari, in cliff time after split 50/50%", async () => {
			await token.mint(accounts[2], 100);
   		await token.approve(staking.address, 1000000, { from: accounts[2] });
			let resultLock = await forTest._stake(staking.address, accounts[2], accounts[3], 62, 10, 2);  //first time stake
      let idLock = eventLockHandler(resultLock);

      let resultBalanceOfValue = await forTest._balanceOf(staking.address, accounts[3]);
      let balanceOf = balanceOfEventHandler(resultBalanceOfValue);
      ...
      await increaseTime(WEEK); //1 week later, cliff works
      resultBalanceOfValue = await forTest._balanceOf(staking.address, accounts[3]);
      balanceOf = balanceOfEventHandler(resultBalanceOfValue);
      assert.equal(balanceOf, 62);
      staking.withdraw({ from: accounts[2] });
			assert.equal(await token.balanceOf(staking.address), 62);				//balance Lock on deposite
		  ...
		  let resultSplit = await forTest._split(idLock, accounts[4], accounts[5], 50, 50 );  //split from accounts[3]
      id = splitEventHandler(resultSplit);
      assert.equal(id.first, 2);
      assert.equal(id.second, 3);
      resultBalanceOfValueAccount_3  = await forTest._balanceOf(staking.address, accounts[3]); //for check balance accounts[3]
      balanceOf = balanceOfEventHandler(resultBalanceOfValueAccount_3);
      ...
      resultBalanceOfValue = await forTest._balanceOf(staking.address, accounts[4]); //for check balance accounts[4]
      balanceOf = balanceOfEventHandler(resultBalanceOfValue);
      assert.equal(balanceOf, 31);    //stRary balance accounts[4], after _split
      resultBalanceOfValue = await forTest._balanceOf(staking.address, accounts[5]); //for check balance accounts[4]
      balanceOf = balanceOfEventHandler(resultBalanceOfValue);
      assert.equal(balanceOf, 31);    //stRary balance accounts[5], after _split

  		await increaseTime(WEEK); //1 week later cliff works
      resultBalanceOfValue = await forTest._balanceOf(staking.address, accounts[4]);
      balanceOf = balanceOfEventHandler(resultBalanceOfValue);
      assert.equal(balanceOf, 31);
      resultBalanceOfValue = await forTest._balanceOf(staking.address, accounts[5]);
      balanceOf = balanceOfEventHandler(resultBalanceOfValue);
      assert.equal(balanceOf, 31);
      ...
      staking.withdraw({ from: accounts[2] });
		...
		it("Test.2. split() and check balance delegated stRari, in slope time after split 90/10%", async () => {
			await token.mint(accounts[2], 100);
   		await token.approve(staking.address, 1000000, { from: accounts[2] });
			let resultLock = await forTest._stake(staking.address, accounts[2], accounts[3], 60, 10, 2);  //first time stake
      let idLock = eventLockHandler(resultLock);
      ...
      staking.withdraw({ from: accounts[2] });
		  ...
		  let resultSplit = await forTest._split(idLock, accounts[4], accounts[5], 90, 10 );  //split from accounts[3]
      id = splitEventHandler(resultSplit);
      assert.equal(id.first, 2);
      assert.equal(id.second, 3);
      resultBalanceOfValueAccount_3  = await forTest._balanceOf(staking.address, accounts[3]); //for check balance accounts[3]
      ...
      staking.withdraw({ from: accounts[2] });
		...
		it("Test.3. split() and check balance delegated stRari, in tail time after split 25/75%", async () => {
			await token.mint(accounts[2], 100);
   		await token.approve(staking.address, 1000000, { from: accounts[2] });
			let resultLock = await forTest._stake(staking.address, accounts[2], accounts[3], 36, 20, 2);  //first time stake
      let idLock = eventLockHandler(resultLock);
      ...
      await increaseTime(WEEK*3); //1 week later, slope works
      resultBalanceOfValueTmp = await forTest._balanceOf(staking.address, accounts[3]);
      balanceOf = balanceOfEventHandler(resultBalanceOfValueTmp);
      ...
      staking.withdraw({ from: accounts[2] });
		  ...
		  let resultSplit = await forTest._split(idLock, accounts[4], accounts[5], 25, 75 );  //split from accounts[3]
      id = splitEventHandler(resultSplit);
      assert.equal(id.first, 2);
      assert.equal(id.second, 3);
      resultBalanceOfValueAccount_3  = await forTest._balanceOf(staking.address, accounts[3]); //for check balance accounts[3]
