...
describe("RoyaltiesRegistryPermissioned in hardhat-deploy", function () {
    ...
    this.beforeAll(async function () {
        ...
        await (await registry.connect(owner).setRoyaltiesAllowed(erc721.address, false, {
            nonce: await getAndIncrementNonce(owner),
            gasPrice,
        })).wait(numberOfBlocksToWait);

        await (await registry.connect(owner).setRoyaltiesByToken(erc721.address, [{account: royaltyRecipient.address, value: 1000}], {
            nonce: await getAndIncrementNonce(owner),
            gasPrice,
        })).wait(numberOfBlocksToWait);
    ...
    describe("getRoyalties Scenarios - Not Allowed (after disabling)", function () {
        it("ERC721 with royalties V2 interface after disabling: returns empty array", async function () {
    ...
    describe("should trade without royalties", function () {
        it("should trade without royalties", async function () {
            ...
            expect(sellerBalanceAfter).to.be.closeTo(sellerBalanceBefore.add(price).sub(feeSellerAmount), ethers.utils.parseEther("0.000001"), "Seller should receive exactly the price");
            ...
            expect(royaltyRecipientBalanceAfter).to.equal(royaltyRecipientBalanceBefore, "Royalty must not be received");
        });
    });
});