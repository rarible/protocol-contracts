...
import { RoyaltiesRegistryPermissioned, TestERC721WithRoyaltiesV1OwnableUpgradeable, TestERC721WithRoyaltiesV2OwnableUpgradeable, TestERC721WithRoyaltyV2981, TestERC721RoyaltiesV2, TestERC721, RoyaltiesRegistryPermissioned__factory, RoyaltiesRegistry } from "../../typechain-types";
...
describe("RoyaltiesRegistryPermissioned in hardhat-deploy", function () {
    ...
    describe("should trade with royalties", function () {
        it("should trade with royalties", async function () {
            ...
            // Assert balance changes exactly, assuming no protocol fees deducted from price
            expect(sellerBalanceAfter).to.be.closeTo(sellerBalanceBefore.add(price).sub(feeSellerAmount).sub(royaltyAmount), ethers.utils.parseEther("0.000001"), "Seller should receive exactly the price minus fees");
            expect(buyerBalanceAfter).to.be.closeTo(buyerBalanceBefore.sub(price).sub(gasCost).add(feeBuyerAmount), ethers.utils.parseEther("0.000001"), "Buyer should pay exactly the price minus gas cost");
