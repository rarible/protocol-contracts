...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  //get bytecode to create RaribleExchangeWrapper
  const ContractFactory = await hre.ethers.getContractFactory("RaribleExchangeWrapper");

  let exchangeV2 = (await hre.deployments.get("ExchangeV2")).address;
  ...
  const dtx = await ContractFactory.getDeployTransaction(settings.marketplaces, settings.weth, settings.transferProxies);
  //console.dir(dtx, {depth:null})
  var fs = require('fs');
  fs.writeFileSync('./RaribleExchangeWrapper.txt', dtx.data , 'utf-8');

  //predict address
  const salt = getSalt();
  let expectedAddress = await factory.getDeploymentAddress(dtx.data, salt)
  let contract = await hre.ethers.getContractAt('RaribleExchangeWrapper', expectedAddress);
  console.log("9.2.! Predict address for RaribleExchangeWrapper: ", expectedAddress)
  
  //prepare ownership transfer calldata
  const ownershipCalldata = await prepareTransferOwnershipCalldata(hre);
  console.log("9.3.! transfer ownership calldata for RaribleExchangeWrapper: ", ownershipCalldata)

  /*
  if (hre.network.name === "hardhat") {
    contract = await ContractFactory.deploy(settings.marketplaces, settings.weth, settings.transferProxies);
  ...
  */
  ...
  //check RaribleExchangeWrapper
  //console.log("RaribleExchangeWrapper owner:", await contract.owner())

  //save artifact
  await hre.deployments.save("RaribleExchangeWrapper", {
    address: expectedAddress,
    ...(await hre.deployments.getExtendedArtifact("RaribleExchangeWrapper"))
  })
