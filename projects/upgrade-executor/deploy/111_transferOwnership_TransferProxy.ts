...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const contractAddress = settings.TransferProxy;
  const oldOwner = signer.address;
  const newOwner = (await hre.deployments.get("UpgradeExecutor")).address;

  console.log(`Transfering ownership of contract: TransferProxy, at:${contractAddress}`)
...
export default func;
func.tags = ['ownership-transfer', 'TransferProxy'];
