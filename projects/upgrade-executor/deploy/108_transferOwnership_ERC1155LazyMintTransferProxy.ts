...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const contractAddress = settings.ERC1155LazyMintTransferProxy;
  const oldOwner = signer.address;
  const newOwner = (await hre.deployments.get("UpgradeExecutor")).address;

  console.log(`Transfering ownership of contract: ERC1155LazyMintTransferProxy, at:${contractAddress}`)
...
export default func;
func.tags = ['ownership-transfer', 'ERC1155LazyMintTransferProxy'];
