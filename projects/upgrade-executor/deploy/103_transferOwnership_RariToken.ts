...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const contractAddress = settings.RariToken;
  const oldOwner = signer.address;
  const newOwner = (await hre.deployments.get("UpgradeExecutor")).address;

  console.log(`Transfering ownership of contract: RariToken, at:${contractAddress}`)
...
export default func;
func.tags = ['ownership-transfer', 'RariToken'];
