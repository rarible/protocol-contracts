...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
    ...
    // Example NFT
    const erc721Address = "0x9A4A7c4D3892A3315B4b60f8Dd3202524F10E861";
    const erc721: ERC721RaribleMinimal = ERC721RaribleMinimal__factory.connect(erc721Address, signer);
    const tokenId = BigNumber.from("113684458893483085791140453563089956290380401423266949679181692179405588135937");
    ...
    //await erc721.setApprovalForAll(exchangeDeployment.address, true);
...
const encodedV3 = ethers.utils.defaultAbiCoder.encode(
    ...
    // Now match
    const tx = await exchange.connect(makerRight).matchOrders(leftOrder, leftSig, rightOrder, rightSig, { gasLimit: 8_000_000, value: 1000 });
