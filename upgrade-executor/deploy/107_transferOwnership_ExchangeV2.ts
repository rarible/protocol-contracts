...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const contractAddress = settings.ExchangeV2;
  const oldOwner = signer.address;
  const newOwner = (await hre.deployments.get("UpgradeExecutor")).address;

  console.log(`Transfering ownership of contract: ExchangeV2, at:${contractAddress}`)
...
export default func;
func.tags = ['ownership-transfer', 'ExchangeV2'];
