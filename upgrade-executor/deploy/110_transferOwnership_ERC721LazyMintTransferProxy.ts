...
const func: DeployFunction = async function (hre: HardhatRuntimeEnvironment) {
  ...
  const contractAddress = settings.ERC721LazyMintTransferProxy;
  const oldOwner = signer.address;
  const newOwner = (await hre.deployments.get("UpgradeExecutor")).address;

  console.log(`Transfering ownership of contract: ERC721LazyMintTransferProxy, at:${contractAddress}`)
...
export default func;
func.tags = ['ownership-transfer', 'ERC721LazyMintTransferProxy'];
