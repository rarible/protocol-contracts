...
const MetaTx = artifacts.require("MetaTxTest.sol");
const NoMetaTx = artifacts.require("NoMetaTxTest.sol");
...
contract("EIP712MetaTransactionDirectTesting", function ([_, owner, account1]) {
//  let testContract
//  let noMetaTest;
//  let transferProxy;
...
//  let community = account1;
  ...
  before('before', async function () {
    noMetaTest = await deployProxy(NoMetaTx, ["NNoMetaTx", "1"], { initializer: '__NoMetaTxTest_init' });
    metaTest = await deployProxy(MetaTx, ["MetaTx", "1"], { initializer: '__MetaTxTest_init' });

    domainData = {
      name: "MetaTx",
      version: "1",
      verifyingContract: metaTest.address,
      chainId: 1337
    };
  ...
  describe("Check Methods", function () {
  	it("upgrade, which use MetaTransaction  works", async () => {
  		const wrapper = await MetaTx.at(noMetaTest.address);
  		await expectThrow(
  			wrapper.getNonce(ZERO_ADDRESS)
  		);

  		await upgradeProxy(noMetaTest.address, MetaTx);
