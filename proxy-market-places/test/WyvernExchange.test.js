...
contract("WyvernExchangeProxy", accounts => {
  ...
  let erc20;
  const feeProtocolReciever = accounts[3];
...
//    erc20 = await ERC20.new(); //todo understand problems with deploy
    ...
    wyvernExchangeProxy.__WyvernExchangeProxy_init(wyvernExchangeWithBulkCancellations.address, 300);
    await wyvernExchangeProxy.setFeeReceiver(feeProtocolReciever, {from: accounts[0]});
  ...
  describe("Check methods through wyvernExchange emulator ", () => {
    it("NFT <-> ETH  matches and transfer", async () => {
      const{addrs, uints, feeMethodsSidesKindsHowToCalls, calldataBuy, calldataSell, replacementPatternBuy, replacementPatternSell, staticExtradataBuy, staticExtradataSell, vs, rssMetadata} = await prepareWywernNFT_ETH();
      console.log("uints", uints[13]);
      await verifyBalanceChange(feeProtocolReciever, -3, () =>
    ...
    async function prepareWywernNFT_ETH() {
    	const addrs = [
          ...
          "0x9133d618c4f756dc231462a70701757d0af9f1bb",
          "0x0000000000000000000000000000000000000000",
          "0x5b3256965e7c3cf26e11fcaf296dfc8807c01073", //addr2 (10)
          "0x45b594792a5cdc008d0de1c1d69faa3d16b3ddc1",
          "0x0000000000000000000000000000000000000000",
          "0x0000000000000000000000000000000000000000"
    	];
