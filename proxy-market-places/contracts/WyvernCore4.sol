...
library ArrayUtils {
    ...
    function guardedArrayReplace(
        ...
        require(array.length == desired.length, "r3");
        require(array.length == mask.length, "r4");
...
contract TokenRecipient {
    ...
    function receiveApproval(
        ...
        ERC20 t = ERC20(token);
        require(t.transferFrom(from, address(this), value), "r5");
...
//            "r6"
//        );
//        require(keccak256(bytes(name)) == _NAME_HASH, "r7");
//        require(keccak256(bytes(version)) == _VERSION_HASH, "r8");
//        require(
//            keccak256(
//                "Order(address exchange,address maker,address taker,uint256 makerRelayerFee,uint256 takerRelayerFee,uint256 makerProtocolFee,uint256 takerProtocolFee,address feeRecipient,uint8 feeMethod,uint8 side,uint8 saleKind,address target,uint8 howToCall,bytes _calldata,bytes replacementPattern,address staticTarget,bytes staticExtradata,address paymentToken,uint256 basePrice,uint256 extra,uint256 listingTime,uint256 expirationTime,uint256 salt,uint256 nonce)"
//            ) == _ORDER_TYPEHASH,
//            "r9"
//        );
//        require(DOMAIN_SEPARATOR == _deriveDomainSeparator(), "r10");
    }
...
//    }
    ...
    function requireValidOrder(
        ...
        require(validateOrder(hash, order, sig), "r12");
        return hash;
    }
...
//                msg.gas, // forward all available gas
//                orderMaker, // call the order maker
//                add(isValidSignatureData, 0x20), // _calldata offset comes after length
//                mload(isValidSignatureData), // load _calldata length
...
//        }
        ...
        internal
    {
        ...
        require(msg.sender == order.maker, "r13");

        /* Calculate order hash. */
        bytes32 hash = hashToSign(order, nonces[order.maker]);

        /* Assert order has not already been approved. */
        require(_approvedOrdersByNonce[hash] == 0, "r14");
    ...
    function cancelOrder(
        ...
        require(msg.sender == order.maker, "r15");
        ...
        returns (uint256)
    {
        ...
        /* Require price cross. */
        require(buyPrice >= sellPrice, "r16");
...
//            revert("SKS_03");
            ...
            require(sell.takerRelayerFee <= buy.takerRelayerFee, "r18");

            if (sell.feeMethod == FeeMethod.SplitFee) {
                /* Assert taker fee is less than or equal to maximum fee specified by buyer. */
                require(sell.takerProtocolFee <= buy.takerProtocolFee, "r19");
                ...
                if (sell.makerRelayerFee > 0) {
                    ...
                    if (sell.paymentToken == address(0)) {
                        ...
                        address payable feeRecipientAddress = payable(sell.feeRecipient);
//                        revert("SKS_04");
...
//                        revert(string(abi.encodePacked("emit rest: ", toString(feeRecipientAddress))));
                        feeRecipientAddress.transfer(makerRelayerFee);
//                        revert("SKS_05");
...
//                        revert("SKS_03_07");
                ...
                if (sell.takerRelayerFee > 0) {
                    ...
                    if (sell.paymentToken == address(0)) {
                        ...
                        address payable feeRecipientAddress = payable (sell.feeRecipient);
                        feeRecipientAddress.transfer(takerRelayerFee);
//                        sell.feeRecipient.transfer(takerRelayerFee);
                ...
                if (sell.makerProtocolFee > 0) {
                    ...
                    if (sell.paymentToken == address(0)) {
                        ...
                        address payable protocolFeeRecipientAddress = payable (protocolFeeRecipient);
                        protocolFeeRecipientAddress.transfer(makerProtocolFee);
                    } else {
                        transferTokens(
                            sell.paymentToken,
                            sell.maker,
                ...
                if (sell.takerProtocolFee > 0) {
                    ...
                    if (sell.paymentToken == address(0)) {
                        ...
                        address payable protocolFeeRecipientAddress = payable (protocolFeeRecipient);
                        protocolFeeRecipientAddress.transfer(takerProtocolFee);
                    } else {
                        transferTokens(
                            sell.paymentToken,
                            buy.maker,
            ...
            require(buy.takerRelayerFee <= sell.takerRelayerFee, "r20");

            if (sell.feeMethod == FeeMethod.SplitFee) {
                /* The Exchange does not escrow Ether, so direct Ether can only be used to with sell-side maker / buy-side taker orders. */
                require(sell.paymentToken != address(0), "r21");

                /* Assert taker fee is less than or equal to maximum fee specified by seller. */
                require(buy.takerProtocolFee <= sell.takerProtocolFee, "r22");
                uint256 makerRelayerFee;
        ...
        if (sell.paymentToken == address(0)) {
            ...
            require(msg.value >= requiredAmount, "r23");
            address payable makerAddress = payable (sell.maker);
            makerAddress.transfer(receiveAmount);
//            sell.maker.transfer(receiveAmount);
            /* Allow overshoot for variable-price auctions, refund difference. */
            uint256 diff = SafeMath.sub(msg.value, requiredAmount);
            if (diff > 0) {
                makerAddress = payable (buy.maker);
                makerAddress.transfer(diff);
//                buy.maker.transfer(diff);
    ...
    /**
     * @dev Return whether or not two orders can be matched with each other by basic parameters (does not check order signatures / _calldata or perform static calls)
     * @param buy Buy-side order
     * @param sell Sell-side order
    ...
    function atomicMatch(
        ...
        //        require(validateOrderParameters(buy), "r24");
        //        } else {
        ...
        //        require(validateOrderParameters(sell), "r25");
        //        } else {
        //        sellHash = _requireValidOrderWithNonce(sell, sellSig);
        //        }

        /* Must be matchable. */
//        require(ordersCanMatch(buy, sell), "r26");
...
//        require(size > 0, "r27");
        ...
        /* Must match _calldata after replacement, if specified. */
...
//        require(ArrayUtils.arrayEq(buy._calldata, sell._calldata), "r28");

        /* Retrieve delegateProxy contract. */
//        OwnableDelegateProxy delegateProxy = registry.proxies(sell.maker);

        /* Proxy must exist. */
//        require(delegateProxy != address(0), "r29");
...
//        require(proxy.proxy(sell.target, sell.howToCall, sell._calldata), "r31");
