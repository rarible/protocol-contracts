...
const AssetMatcherWETHTest = artifacts.require("AssetMatcherWETHTest.sol");

const { Order, Asset, sign } = require("../order");
const EIP712 = require("../EIP712");
const ZERO = "0x0000000000000000000000000000000000000000";
const { expectThrow, verifyBalanceChange } = require("@daonomic/tests-common");
const { ETH, ERC20, ERC721, ERC1155, ORDER_DATA_V1, ORDER_DATA_V2, TO_MAKER, TO_TAKER, PROTOCOL, ROYALTY, ORIGIN, PAYOUT, CRYPTO_PUNKS, WETH_UNWRAP, COLLECTION, TO_LOCK, LOCK, enc, id } = require("../assets");
...
contract("ExchangeV2, sellerFee + buyerFee =  6%,", accounts => {
  ...
  let assetMatcherWETHTest;
	...
	const resetState = async () => {
		...
		/*this way msg.sender.transfer(value) in wETH9 token get exception: "revert"*/
//		transferManagerTest = await deployProxy(RaribleTransferManagerTest, [community, royaltiesRegistry.address, transferProxy.address, erc20TransferProxy.address], { initializer: "__RaribleTransferManagerTest_init" });
    /*this way msg.sender.transfer(value) works good*/
    transferManagerTest = await RaribleTransferManagerTest.new();
    await transferManagerTest.__RaribleTransferManagerTest_init(community, royaltiesRegistry.address, transferProxy.address, erc20TransferProxy.address);
		testing = await deployProxy(ExchangeV2, [transferManagerTest.address, protocolFee], { initializer: "__ExchangeV2_init" });
  ...
  describe("check WETH", () => {
    it("matchOrders ERC721<-> WETH9 (WETH_UNWRAP) protocol fee, origin, royalties, payouts ", async () => {
      await erc721.mint(accounts[1], erc721TokenId1);
      await erc721.setApprovalForAll(transferProxy.address, true, {from: accounts[1]});
      await tokenWETH9.deposit({from: accounts[2], value: 1000});
...
//      console.log("Used ERC721<->WETH_UNWRAP for matchOrders (protocol fee, origin, royalties, payouts), estimate Gas: ", tx.receipt.gasUsed);
    ...
    it("matchOrders ERC721<-> ERC20 (WETH) protocol fee, origin, royalties, payouts ", async () => {
      await erc721.mint(accounts[1], erc721TokenId1);
      await erc721.setApprovalForAll(transferProxy.address, true, {from: accounts[1]});
      await tokenWETH9.deposit({from: accounts[2], value: 1000});
      await tokenWETH9.approve(erc20TransferProxy.address, 1000, {from: accounts[2]});
    ...
    it("matchOrders ERC721<-> WETH9 (WETH_UNWRAP) only protocol fee ", async () => {
      await erc721.mint(accounts[1], erc721TokenId1);
      await erc721.setApprovalForAll(transferProxy.address, true, {from: accounts[1]});
      await tokenWETH9.deposit({from: accounts[2], value: 1000});
      await tokenWETH9.approve(erc20TransferProxy.address, 1000, {from: accounts[2]});
...
//      console.log("Used ERC721<->WETH_UNWRAP for matchOrders (protocol fee), estimate Gas: ", tx.receipt.gasUsed);
    ...
    it("matchOrders ERC721<-> ERC20 (WETH) only protocol fee ", async () => {
      ...
      const left = Order(accounts[1], Asset(ERC721, enc(erc721.address, erc721TokenId1), 1), ZERO, Asset(ERC20, enc(tokenWETH9.address), 200), 1, 0, 0, "0xffffffff", "0x");
      const right = Order(accounts[2], Asset(ERC20, enc(tokenWETH9.address), 200), ZERO, Asset(ERC721, enc(erc721.address, erc721TokenId1), 1), 1, 0, 0, "0xffffffff", "0x");
