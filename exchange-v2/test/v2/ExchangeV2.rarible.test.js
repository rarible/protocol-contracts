...
const OnChainOrderReentrancyAttacker = artifacts.require("OnChainOrderReentrancyAttacker.sol");
...
//		})
		...
		it("check reentrancy with upsertOrder()", async () => {
			onChainOrderReentrancyAttacker = await OnChainOrderReentrancyAttacker.new(testing.address);
			const makerLeftEthDonate = accounts[1];
			const makerLeft = onChainOrderReentrancyAttacker.address;
			const makerRight = accounts[2];
			...
			const left = Order(makerLeft, Asset(ETH, "0x", 200), ZERO, Asset(ERC20, enc(t1.address), 100), 1, 0, 0, "0xffffffff", "0x");
			const leftNew = Order(makerLeft, Asset(ETH, "0x", 100), ZERO, Asset(ERC20, enc(t1.address), 100), 1, 0, 0, "0xffffffff", "0x");
			const right = Order(makerRight, Asset(ERC20, enc(t1.address), 50), ZERO, Asset(ETH, "0x", 100), 1, 0, 0, "0xffffffff", "0x");

      await onChainOrderReentrancyAttacker.setOrder(left);
      await onChainOrderReentrancyAttacker.upsert({ from: makerLeftEthDonate, value: 500, gasPrice: 0 });
      assert.equal(await onChainOrderReentrancyAttacker.getBalance(), 294);

      await onChainOrderReentrancyAttacker.setOrder(leftNew);
//      await onChainOrderReentrancyAttacker.attack({ from: makerLeftEthDonate, value: 500, gasPrice: 0 });
//      assert.equal(await onChainOrderReentrancyAttacker.getBalance(), 394);
