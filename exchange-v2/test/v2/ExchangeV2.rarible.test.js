...
//  });
  ...
  describe("check WETH", () => {
    it("ERC721 to WETH order maker  ", async () => {
      await runTest(async createOrder => {
        ...
        await testing.setAssetMatcher(WETH_UNWRAP, assetMatcherWETHTest.address);
      	...
      	const right = Order(accounts[2], Asset(ERC20, enc(tokenWETH.address), 200), ZERO, Asset(ERC721, enc(erc721.address, erc721TokenId1), 1), 1, 0, 0, "0xffffffff", "0x");
      	...
      	await verifyBalanceChange(accounts[2], 206, async () =>
      		verifyBalanceChange(accounts[1], -194, async () =>
      			verifyBalanceChange(protocol, -12, () =>
      				testing.matchOrders(left, dataLeft.signature, right, dataRight.signature)
      			)
      		)
      	)
