...
contract("Onchain orders ", accounts => {
    let testing;
    let transferProxy;
    ...
    describe("onchain orders creation", () => {
        it("should create, update then cancel order", async () => {
            ...
            const existanceBeforeCreation = await testing.checkOrderExistance(order);
            ...
            const existanceAfterCreation = await testing.checkOrderExistance(order)
            ...
            const existanceAfterCanceling = await testing.checkOrderExistance(updatedOrder)
            assert.equal(existanceAfterCanceling, false, "existance after creation")
        })
    ...
    describe("matchOrders", () => {
        ...
        it("left order is ETH => erc20 onchain, right is erc20 => ETH offchain", async () => {
            const tokenHolder = accounts[1];
            const ethHolder = accounts[2];

            await t1.mint(tokenHolder, 100);
            await t1.approve(erc20TransferProxy.address, 10000000, { from: tokenHolder });

            const left = Order(ethHolder, Asset(ETH, "0x", 200), ZERO, Asset(ERC20, enc(t1.address), 100), 1, 0, 0, "0xffffffff", "0x");
            const right = Order(tokenHolder, Asset(ERC20, enc(t1.address), 100), ZERO, Asset(ETH, "0x", 200), 1, 0, 0, "0xffffffff", "0x");

            await verifyBalanceChange(ethHolder, 206, async () =>
                verifyBalanceChange(testing.address, -206, () =>
                    testing.upsertOrder(left, { from: ethHolder, value: 300, gasPrice: 0 })
                )
            )

            await verifyBalanceChange(tokenHolder, -194, async () =>
                verifyBalanceChange(protocol, -12, () =>
                    testing.matchOrders(left, "0x", right, "0x", { from: tokenHolder, value: 0, gasPrice: 0 })
                )
            )

            assert.equal(await t1.balanceOf(tokenHolder), 0);
            assert.equal(await t1.balanceOf(ethHolder), 100);
        })

        it("left order is ETH => erc20 onchain, right is erc20 => ETH onchain", async () => {
            const tokenHolder = accounts[1];
            const ethHolder = accounts[2];

            await t1.mint(tokenHolder, 110);
            await t1.approve(erc20TransferProxy.address, 11000000, { from: tokenHolder });
            ...
            await testing.upsertOrder(right, { from: tokenHolder, gasPrice: 0 });
            ...
            assert.equal(await t1.balanceOf(tokenHolder), 10);
            assert.equal(await t1.balanceOf(ethHolder), 100);
        })

        it("left order is ETH => erc721 onchain, right is erc721 => ETH onchain", async () => {
            const tokenHolder = accounts[1];
            const ethHolder = accounts[2];

            await erc721.mint(tokenHolder, erc721TokenId1);
            await erc721.setApprovalForAll(transferProxy.address, true, { from: tokenHolder });

            const left = Order(ethHolder, Asset(ETH, "0x", 200), ZERO, Asset(ERC721, enc(erc721.address, erc721TokenId1), 1), 1, 0, 0, "0xffffffff", "0x");
            const right = Order(tokenHolder, Asset(ERC721, enc(erc721.address, erc721TokenId1), 1), ZERO, Asset(ETH, "0x", 200), 1, 0, 0, "0xffffffff", "0x");
            ...
            assert.equal(await erc721.balanceOf(tokenHolder), 0);
            assert.equal(await erc721.balanceOf(ethHolder), 1);
        })

        it("left order is ETH => erc1155 onchain, right is erc1155 => ETH onchain", async () => {
            const tokenHolder = accounts[1];
            const ethHolder = accounts[2];

            await erc1155_v2.mint(tokenHolder, erc1155TokenId1, [], 100);
            await erc1155_v2.setApprovalForAll(transferProxy.address, true, { from: tokenHolder });

            const left = Order(ethHolder, Asset(ETH, "0x", 200), ZERO, Asset(ERC1155, enc(erc1155_v2.address, erc1155TokenId1), 100), 1, 0, 0, "0xffffffff", "0x");
            const right = Order(tokenHolder, Asset(ERC1155, enc(erc1155_v2.address, erc1155TokenId1), 100), ZERO, Asset(ETH, "0x", 200), 1, 0, 0, "0xffffffff", "0x");
            ...
            assert.equal(await erc1155_v2.balanceOf(tokenHolder, erc1155TokenId1), 0);
            assert.equal(await erc1155_v2.balanceOf(ethHolder, erc1155TokenId1), 100);
        })
    ...
    describe("Do matchOrders(), orders dataType == V1", () => {
        it("From ERC20(100) to ERC20(200) Protocol, Origin fees, no Royalties ", async () => {
            ...
            await testing.upsertOrder(left, { from: accounts[1], gasPrice: 0 });

            await testing.matchOrders(left, "0x", right, "0x", { from: accounts[2] });
        ...
        it("From ERC1155(RoyalytiV2, DataV1) to ERC20(DataV1):Protocol, Origin fees, Royalties ", async () => {
            ...
            await testing.upsertOrder(left, { from: accounts[2], gasPrice: 0 });

            await testing.matchOrders(left, "0x", right, "0x", { from: accounts[1] });
        ...
        it("From ETH(DataV1) to ERC720(RoyalytiV1, DataV1) Protocol, Origin fees, Royalties", async () => {
            ...
            await testing.upsertOrder(right, { from: accounts[1], gasPrice: 0 });
                            ...
                            verifyBalanceChange(accounts[5], -10, async () =>
                                verifyBalanceChange(accounts[6], -12, async () =>
                                    verifyBalanceChange(accounts[7], -14, async () =>
                                        verifyBalanceChange(protocol, -12, () =>
                                            testing.matchOrders(left, "0x", right, "0x", { from: accounts[2], value: 300, gasPrice: 0 })
                                        )
                                    )
                                )
                            )
    ...
    describe("Do matchOrders(), orders dataType == V1, MultipleBeneficiary", () => {
		...
		it("From ERC721(DataV1) to ERC20(NO DataV1) Protocol, Origin fees, no Royalties, payouts: 110%, throw", async () => {
			...
			await expectThrow(
				testing.matchOrders(left, "0x", right, "0x", { from: accounts[2] })
			);
    ...
    describe("Catch emit event Transfer", () => {
		it("From ETH(DataV1) to ERC721(DataV1) Protocol, check emit ", async () => {
            ...
            await testing.upsertOrder(right, { from: seller, gasPrice: 0 });
			let tx = await testing.matchOrders(left, "0x", right, "0x", { from: buyer, value: 300, gasPrice: 0 });
			let errorCounter = 0
		...
		it("From ERC1155(DataV2) to ETH(DataV1) Protocol, check emit ", async () => {
			...
			await testing.upsertOrder(right, { from: buyer, value: 300, gasPrice: 0 });
			let tx = await testing.matchOrders(left, "0x", right, "0x", { from: seller, value: 300, gasPrice: 0 });
			let errorCounter = 0
			//			eventEmitted  - срабатывает по нескольким transfer, для фиксации ошибки нужно чтоб все трансферы завалились
			truffleAssert.eventEmitted(tx, 'Transfer', (ev) => {
				let result = false;
