...
contract("RaribleExchangeWrapper WETH purchases", accounts => {
  ...
  beforeEach(async () => {
    ...
    //deploy wrapper
    bulkExchange = await ExchangeBulkV2.new([ZERO_ADDRESS, exchangeV2.address, seaport.address, ZERO_ADDRESS, ZERO_ADDRESS, ZERO_ADDRESS, seaport.address, ZERO_ADDRESS, ZERO_ADDRESS], weth.address, [erc20TransferProxy.address, seaport.address]);
  })
  ...
  it("batch order: WETH rarible, WETH seaport + ETH rarible + ETH seaport", async () => {
    ...
    const tradeDataSeaPort1 = PurchaseData(6, price, await encodeCurrencyAndDataTypeAndFees(1, 0, 1500, 500), dataForSeaportWithSelector);
    ...
    const tradeDataSeaPort2 = PurchaseData(6, price, await encodeCurrencyAndDataTypeAndFees(0, 0, 1500, 500), dataForSeaportWithSelector2);
  ...
  it("batch order: WETH rarible, WETH seaport (reverted) + ETH rarible + ETH seaport (reverted)", async () => {
    ...
    const tradeDataSeaPort1 = PurchaseData(6, price, await encodeCurrencyAndDataTypeAndFees(1, 0, 1500, 500), "0x");
    ...
    const tradeDataSeaPort2 = PurchaseData(6, price, await encodeCurrencyAndDataTypeAndFees(0, 0, 1500, 500), "0x");
