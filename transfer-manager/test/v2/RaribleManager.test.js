...
const { ETH, ERC20, ERC721, ERC1155, ORDER_DATA_V1, ORDER_DATA_V2, TO_MAKER, TO_TAKER, PROTOCOL, ROYALTY, ORIGIN, PAYOUT, CRYPTO_PUNK, WETH_UNWRAP, COLLECTION, enc, encDataV2, id } = require("@rarible/exchange-v2/test/assets");
...
contract("RaribleTransferManagerTest:doTransferTest()", accounts => {
  ...
  const protocolCommission = 300;
    ...
    await erc1155V2.initialize();
		...
		/*for wrapped ETH, which need to unwrap if assetClass == WETH_UNWRAP*/
		tokenWETH = await wETHTest.new();
	});

	describe("Check doTransfers()", () => {

		it("Transfer from ERC721 to WETH_UNWRAP protocol fee 6%", async () => {
			const tokenWETHOwner = accounts[2];
			const { left, right, leftSide, rightSide, matchFees } = await prepare721_WETH_UNWRAP_Orders();
      const ethSender = ZERO;

//      const tx = await testing.doTransfers(leftSide, rightSide, matchFees.feeSide, ethSender);
//      console.log("Used ERC721<->WETH_UNWRAP, estimate Gas: ", tx.receipt.gasUsed);
      await verifyBalanceChange(tokenWETHOwner, 0, () =>
      	verifyBalanceChange(accounts[1], -97, () =>
      	  verifyBalanceChange(testing.address, 0, () =>
			...
			assert.equal(await tokenWETH.balanceOf(tokenWETHOwner), 897);
			assert.equal(await erc721.balanceOf(accounts[1]), 0);
			assert.equal(await erc721.balanceOf(tokenWETHOwner), 1);
		})

		async function prepare721_WETH_UNWRAP_Orders() {
			await erc721.mint(accounts[1], erc721TokenId1);
			await erc721.setApprovalForAll(transferProxy.address, true, {from: accounts[1]});
      await tokenWETH.deposit({from: accounts[2], value: 1000});
      await tokenWETH.approve(erc20TransferProxy.address, 1000, {from: accounts[2]});
      assert.equal(await tokenWETH.balanceOf(accounts[2]), 1000);

			const left = Order(accounts[1], Asset(ERC721, enc(erc721.address, erc721TokenId1), 1), ZERO, Asset(WETH_UNWRAP, enc(tokenWETH.address), 100), 1, 0, 0, "0xffffffff", "0x");
			const right = Order(accounts[2], Asset(WETH_UNWRAP, enc(tokenWETH.address), 100), ZERO, Asset(ERC721, enc(erc721.address, erc721TokenId1), 1), 1, 0, 0, "0xffffffff", "0x");
      ...
      const matchFees = await testing.getFeeSide.call(left, right);
		...
		it("Transfer from ERC721 to WETH protocol fee 6%", async () => {
			const tokenWETHOwner = accounts[2];
			const { left, right, leftSide, rightSide, matchFees } = await prepare721_WETH_Orders();
      const ethSender = ZERO;

      const tx = await testing.doTransfers(leftSide, rightSide, matchFees.feeSide, ethSender);
//      console.log("Used ERC721<->WETH, estimate Gas: ", tx.receipt.gasUsed);
//      await verifyBalanceChange(tokenWETHOwner, 0, () =>
//      	verifyBalanceChange(accounts[1], -97, () =>
//      	  verifyBalanceChange(testing.address, 0, () =>
//        	  verifyBalanceChange(protocol, -6, () =>
...
//      );
		...
		async function prepare721_WETH_Orders(t1Amount = 106) {
			await erc721.mint(accounts[1], erc721TokenId1);
			await erc721.setApprovalForAll(transferProxy.address, true, {from: accounts[1]});
      await tokenWETH.deposit({from: accounts[2], value: 1000});
      await tokenWETH.approve(erc20TransferProxy.address, 1000, {from: accounts[2]});
      assert.equal(await tokenWETH.balanceOf(accounts[2]), 1000);
			...
			const left = Order(accounts[1], Asset(ERC721, enc(erc721.address, erc721TokenId1), 1), ZERO, Asset(ERC20, enc(tokenWETH.address), 100), 1, 0, 0, "0xffffffff", "0x");
			const right = Order(accounts[2], Asset(ERC20, enc(tokenWETH.address), 100), ZERO, Asset(ERC721, enc(erc721.address, erc721TokenId1), 1), 1, 0, 0, "0xffffffff", "0x");
      ...
      const matchFees = await testing.getFeeSide.call(left, right);
		...
		it("Transfer ERC721<->ETH, protocol fee 6% ", async () => {
			const { left, right, leftSide, rightSide, matchFees } = await prepare721_toETHOrders();
      const ethSender = accounts[0];

//      const tx = await testing.doTransfers(leftSide, rightSide, matchFees.feeSide, ethSender, {value: 300, from: ethSender, gasPrice: 0});
//      console.log("Used erc721<->ETH, estimate Gas: ", tx.receipt.gasUsed);
      await verifyBalanceChange(ethSender, 103, () =>
      	verifyBalanceChange(accounts[1], -97, () =>
        	verifyBalanceChange(protocol, -6, () =>
            testing.doTransfers(leftSide, rightSide, matchFees.feeSide, ethSender,
			...
			assert.equal(await erc721.balanceOf(accounts[1]), 0);
			assert.equal(await erc721.balanceOf(ethSender), 1);
		})

		async function prepare721_toETHOrders() {
			await erc721.mint(accounts[1], erc721TokenId1);
			await erc721.setApprovalForAll(transferProxy.address, true, {from: accounts[1]});
      assert.equal(await erc721.balanceOf(accounts[1]), 1);

      const left = Order(accounts[1], Asset(ERC721, enc(erc721.address, erc721TokenId1), 1), ZERO, Asset(ETH, "0x", 100), 1, 0, 0, "0xffffffff", "0x");
			const right = Order(accounts[0], Asset(ETH, "0x", 100), ZERO, Asset(ERC721, enc(erc721.address, erc721TokenId1), 1), 1, 0, 0, "0xffffffff", "0x");
...
//      left.takeAsset.value = 5;
    ...
    it("Transfer from ERC20 to ERC1155, protocol fee 6% (buyerFee3%, sallerFee3%)", async () => {
      ...
      const ethSender = ZERO;
      const tx = await testing.doTransfers(leftSide, rightSide, matchFees.feeSide, ethSender);
//      console.log("Used erc721<->ERC20, estimate Gas: ", tx.receipt.gasUsed);
