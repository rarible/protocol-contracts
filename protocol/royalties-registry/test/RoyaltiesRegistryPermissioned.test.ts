...
await expect(tx).to.emit(registry, "RoyaltiesAllowedChanged").withArgs(await erc721V1.getAddress(), true);
...
await expect(registry.connect(user).setRoyaltiesAllowed(await erc721V1.getAddress(), true)).to.be.revertedWith("not whitelister");
});
});
...
await registry.connect(owner).setProviderByToken(await erc721V1.getAddress(), await royaltiesProvider.getAddress());
});
