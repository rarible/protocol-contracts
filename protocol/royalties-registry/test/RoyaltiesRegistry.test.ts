...
import { type RoyaltiesRegistry, RoyaltiesRegistry__factory, type RoyaltiesRegistryTest, RoyaltiesRegistryTest__factory, type TestERC721WithRoyaltiesV1OwnableUpgradeable, TestERC721WithRoyaltiesV1OwnableUpgradeable__factory, type TestERC721WithRoyaltiesV2OwnableUpgradeable, TestERC721WithRoyaltiesV2OwnableUpgradeable__factory, type RoyaltiesProviderTest, RoyaltiesProviderTest__factory, type TestERC721RoyaltyV2Legacy, TestERC721RoyaltyV2Legacy__factory, type RoyaltiesProviderV2Legacy, RoyaltiesProviderV2Legacy__factory, type TestERC721ArtBlocks, TestERC721ArtBlocks__factory, type RoyaltiesProviderArtBlocks, RoyaltiesProviderArtBlocks__factory, type TestERC721WithRoyaltyV2981, TestERC721WithRoyaltyV2981__factory, type TestERC721ArtBlocksV2, TestERC721ArtBlocksV2__factory, type RoyaltiesProviderArtBlocksV2, RoyaltiesProviderArtBlocksV2__factory } from "../types/ethers-contracts";
describe("RoyaltiesRegistry, test methods", function () {
  ...
  describe("RoyaltiesRegistry token supports IERC2981:", () => {
    it("Get 10% royalties by token, use RoyaltiesRegistryTest (event) ", async function () {
      ...
      const ERC721_V2981 = await new TestERC721WithRoyaltyV2981__factory(acc1).deploy("Rarible", "RARI", "https://ipfs.rarible.com");
  ...
  it("simple V1 royalties", async function () {
      ...
      const ERC721_V1OwnUpgrd = await new TestERC721WithRoyaltiesV1OwnableUpgradeable__factory(acc2).deploy("Rarible", "RARI", "https://ipfs.rarible.com");
      await ERC721_V1OwnUpgrd.initialize(); //set V1 interface
   await ERC721_V1OwnUpgrd.mint(acc2.address, erc721TokenId1, [{account: acc5.address, value: 1000n}, {account: acc7.address, value: 1200n}]); //set royalties by contract
...
it("simple V2 royalties", async function () {
      ...
      const ERC721_V2OwnUpgrd = await new TestERC721WithRoyaltiesV2OwnableUpgradeable__factory(acc2).deploy("Rarible", "RARI", "https://ipfs.rarible.com");
      await ERC721_V2OwnUpgrd.initialize(); //set V2 interface
   await ERC721_V2OwnUpgrd.mint(acc2.address, erc721TokenId1, [{account: acc5.address, value: 700n}, {account: acc6.address, value: 800n}, {account: acc7.address, value: 900n}, {account: acc8.address, value: 1000n}]); //set royalties by contract
     const part = await royaltiesRegistryTest._getRoyalties(royaltiesRegistry.address, ERC721_V2OwnUpgrd.address, erc721TokenId1);
const receipt = await part.wait();
      const event = receipt.events?.find(e => e.event === "getRoyaltiesTest");
      const royalties = event?.args?.royalties;
...
it("SetRoyaltiesByToken, initialize by Owner, emit get", async function () {
     ...
     await royaltiesRegistry.setRoyaltiesByToken(acc5.address, [{account: acc3.address, value: 600n}, {account: acc4.address, value: 1100n}]); //set royalties by token and tokenId
     await royaltiesRegistry.setRoyaltiesByToken(acc5.address, [{account: acc3.address, value: 600n}, {account: acc4.address, value: 1100n}]); //set royalties by token and tokenId
...
let ownerErc721 = (await ethers.getSigners())[6];
      ...
      const ERC721_V1OwnUpgrd = await new TestERC721WithRoyaltiesV1OwnableUpgradeable__factory(ownerErc721).deploy("Rarible", "RARI", "https://ipfs.rarible.com");
await ERC721_V1OwnUpgrd.initialize(); //set V1 interface
await ERC721_V1OwnUpgrd.mint(acc2.address, erc721TokenId1, []);
     await royaltiesRegistry.setRoyaltiesByToken(ERC721_V1OwnUpgrd.address, [{account: acc3.address, value: 500n}, {account: acc4.address, value: 1000n}], {from: ownerErc721.address}); //set royalties by token and tokenId
const part = await royaltiesRegistryTest._getRoyalties(royaltiesRegistry.address, ERC721_V1OwnUpgrd.address, erc721TokenId1);
const receipt = await part.wait();
      const event = receipt.events?.find(e => e.event === "getRoyaltiesTest");
      const royalties = event?.args?.royalties;
...
const token = await new TestERC721RoyaltyV2Legacy__factory(acc2).deploy("Rarible", "RARI", "https://ipfs.rarible.com");
...
it("SetProviderByToken, initialize by Owner", async function () {
      const [_, __, acc2, acc3, acc4] = await ethers.getSigners();
      const ERC721_V1OwnUpgrd = await new TestERC721WithRoyaltiesV1OwnableUpgradeable__factory(acc2).deploy("Rarible", "RARI", "https://ipfs.rarible.com");
      await testRoyaltiesProvider.initializeProvider(ERC721_V1OwnUpgrd.address, erc721TokenId1, [{account: acc3.address, value: 500n}, {account: acc4.address, value: 1000n}]); //initialize royalties provider
...
let ownerErc721 = (await ethers.getSigners())[6];
      const [_, __, acc2, acc3, acc4] = await ethers.getSigners();
      const ERC721_V1OwnUpgrd = await new TestERC721WithRoyaltiesV1OwnableUpgradeable__factory(ownerErc721).deploy("Rarible", "RARI", "https://ipfs.rarible.com");
      await testRoyaltiesProvider.initializeProvider(ERC721_V1OwnUpgrd.address, erc721TokenId1, [{account: acc3.address, value: 600n}, {account: acc4.address, value: 1100n}]); //initialize royalties provider
...
await royaltiesRegistry.connect(ownerErc721).setProviderByToken(ERC721_V1OwnUpgrd.address, testRoyaltiesProvider.address); //set royalties by provider
const part = await royaltiesRegistryTest._getRoyalties(royaltiesRegistry.address, ERC721_V1OwnUpgrd.address, erc721TokenId1);
const receipt = await part.wait();
      const event = receipt.events?.find(e => e.event === "getRoyaltiesTest");
      const royalties = event?.args?.royalties;
...
await royaltiesRegistry.connect(ownerErc721).setProviderByToken(ERC721_V1OwnUpgrd.address, testRoyaltiesProvider.address); //set royalties by provider
const part = await royaltiesRegistryTest._getRoyalties(royaltiesRegistry.address, ERC721_V1OwnUpgrd.address, erc721TokenId2);
const receipt = await part.wait();
      const event = receipt.events?.find(e => e.event === "getRoyaltiesTest");
      const royalties = event?.args?.royalties;
expect(royalties.length).to.equal(0);
