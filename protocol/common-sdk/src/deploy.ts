...
export async function deployTransparentProxy<T extends BaseContract>(
  ethers: HardhatEthers,
  {
    ...
    implementationArgs = [],
    contractAtName,
  }: DeployTransparentProxyParams<T>,
): Promise<DeployTransparentProxyResult<T>> {
  ...
  const initData = initFunction != null ? implementation.interface.encodeFunctionData(initFunction, initArgs) : "0x";
