...
contract("AssetMatcher", (accounts) => {
  ...
  it("setAssetMatcher works", async () => {
    ...
    await expectThrow(testing.matchAssetsTest(order.AssetType(ERC20, encoded), order.AssetType(id("BLA"), encoded)));
  ...
  describe("CRYPTO_PUNKS", () => {
    it("Punk Id = 3000 <-> Punk Id = 3000 matches!", async () => {
      ...
      const result = await testing.matchAssetsTest(
        order.AssetType(CRYPTO_PUNKS, encodedPunk1),
        order.AssetType(CRYPTO_PUNKS, encodedPunk2),
      );
