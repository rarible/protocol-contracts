...
import { encodeSetProtocolFee } from "../sdk/encodeUtils";
describe("RaribleTransferManager", () => {
    ...
    it("should set protocol fee using encoded calldata", async () => {
        const receiver = protocol.address;
        const buyerAmt = 500;
        const sellerAmt = 400;
        const calldata = encodeSetProtocolFee(receiver, buyerAmt, sellerAmt);
        await (await protocol.sendTransaction({to: rtm.address, data: calldata})).wait();
        const fee = await rtm.protocolFee();
        expect(fee.receiver).to.eq(receiver);
        expect(fee.buyerAmount).to.eq(buyerAmt);
        expect(fee.sellerAmount).to.eq(sellerAmt);
    });
