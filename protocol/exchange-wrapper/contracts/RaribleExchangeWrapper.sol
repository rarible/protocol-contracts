...
contract RaribleExchangeWrapper is Ownable, ERC721Holder, ERC1155Holder, IsPausable {
    ...
    constructor(
        ...
        for (uint i = 0; i < transferProxies.length; ++i) {
            if (_weth != address(0)) {
                IERC20(_weth).approve(transferProxies[i], UINT256_MAX);
            }
        }
    }
    ...
    function singlePurchase(
        ...
        if (wethAmountNeeded > 0) {
            IERC20(weth).transferFrom(_msgSender(), address(this), wethAmountNeeded);
        }
            ...
            for (uint i = 0; i < input.details.length; ++i) {
                ...
                if (input.orders[orderId].delegateType == 1) {
                    ...
                    for (uint256 j = 0; j < pairs.length; j++) {
                        ...
                        IERC721(address(p.token)).safeTransferFrom(address(this), _msgSender(), p.tokenId);
                    }
                    ...
                    for (uint256 j = 0; j < pairs.length; j++) {
                        ...
                        IERC1155(address(p.token)).safeTransferFrom(
                            address(this),
                            _msgSender(),
                            p.tokenId,
                            p.amount,
    ...
    function transferChangeWETH() internal {
        ...
        if (wethAmount > 0) {
            IERC20(weth).transfer(_msgSender(), wethAmount);
        }
    }
    ...
    */
    function approveWETH(address[] calldata transferProxies) external onlyOwner {
        for (uint i = 0; i < transferProxies.length; ++i) {
            IERC20(weth).approve(transferProxies[i], UINT256_MAX);
        }
    }
