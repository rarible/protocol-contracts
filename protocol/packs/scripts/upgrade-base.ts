/// @ai_context
/// Upgrade script that detects implementation changes and upgrades proxies (Base mainnet)
///
/// Env:
/// - HARDHAT_NETWORK=base (or use --network base)
/// - FORCE_UPGRADE (optional): "true" to upgrade even if bytecode matches
/// - UPGRADE_RARIPACK / UPGRADE_NFTPOOL / UPGRADE_PACKMANAGER (optional): scope upgrades
...
async function main() {
  ...
  for (const contract of contractsToCheck) {
    ...
    if (needsUpgrade || forceUpgrade) {
      ...
      const proxyAdmin = "0x" + adminSlotValue.slice(26);
  ...
  for (const { contract, proxyAdmin } of upgradesToPerform) {
    ...
    const paramsPath = path.join(paramsDir, `upgrade${contract.name}.base.json`);
    const params = {
    ...
    const upgradeCmd = `npx hardhat ignition deploy ${moduleFile} --network base --parameters ${paramsPath}`;
