...
//			await assertCurrent([6, 50, 10]);
		...
		it("One line. cliff = 3  change period from 2 to 5, step = 2 (in cliff) ", async () => {
...
//			await assertCurrent([2, 30, 0]);
      ...
      assert.equal(newSlope, 8);
		...
		it("One line. cliff = 3  change period from 2 to 5, step = 5 (out cliff) ", async () => {
			await forTest.add([1, 40, 20], 3);
			await assertCurrent([1, 40, 0]);
			...
			rezultChangePeriod = await forTest.changePeriodTest([1, 40, 20], 3, 5, 5);
			let newSlope;
      truffleAssert.eventEmitted(rezultChangePeriod, 'changePeriodResult', (ev) => {
       	newSlope = ev.result;
        return true;
